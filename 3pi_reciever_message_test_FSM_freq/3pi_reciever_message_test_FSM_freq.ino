...
//smaller message map for testing, make sure that the transmitter and reciver have the same map uncommented
Message messages[] = {
  {"hello", "010010010010"},
  {"bye", "000101010100"},
  {"yes", "001010101010"},
...
void testRoutine() {
  if (millis() - lastSampleTime >= time_per_bit) {
    ...
    if (millis() - testStartTime >= testDuration and messageCountRead==expectedMessageCount ) {
      ...
      Serial.print(millis() - testStartTime);Serial.print(",");
      Serial.println(time_per_bit);
